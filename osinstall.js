...
/** Generate Disk parameters for super simple disk layout: root+swap (and optional boot).
  ...
  */
  function disk_params (f) { // Export
    ...
    disk.ansdisk = ansparr; // OLD: sda, NEW: ansparr
    disk_out_winxml(ansparr);
  ...
  */
  ...
  /** Extract parts into proper sequence here
  * Note 
  * */
function parts2array(parts) {
    ...
    pnames.forEach((pn) => {
      ...
      if (part.links.labels && Array.isArray(part.links.labels)) { part.lbl = part.links.labels[0]; }
      delete(part.links);
      delete(part.holders);
      part.seq = seqi;
   ...
   /** Convert partition size from unit notation to absolute numeric units */
...
function disk_out_winxml(ansparr) {
  ...
  ansparr.forEach((p) => {
    ...
    console.log(Mustache.render(cpart, p));
  });
  ansparr.forEach((p) => {
    
    console.log(Mustache.render(mpart, p));
  });
