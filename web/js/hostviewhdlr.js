...
function multigridview(ev, act) {
  ...
  axios.get(act.url).then(function (resp) { // '/groups'
    ...
    if (!Array.isArray(grps) && (typeof grps == 'object') && grps.data) {
      //if (!Array.isArray(grps.data)) { return toastr.error(`No array found for groups`);  }
      grps = grps.data;
    }
...
function form_jg(fdefs, opts) {
  ...
  fdefs.forEach((fd) => { // map ?
    ...
    // Removed ${idprefix} as it is purely UI-access related
    let ltitle = opts.lpopup ? `title="${prefix}${fd.name}"` : ""; // WIP: Label title (popup)
  ...
  if (opts.layout == 'rowform') {
    ...
    if (opts.mvar && opts.unip) { cont += `<td><button @click.prevent="delentry('${opts.mvar}', ${svar}.${opts.unip})">${symdel}</button></td>`; } // ${svar}
    cont += `</tr>`;
  } // Term. else { cont += `</div>`;}
  if (opts.btitle && opts.bid) {
    let vact = opts.mvar ? `@click.prevent="send()"` : "";
 ...
 * div elements should be marked with attr data-memname to be able to select them.
 * @param fdefs (array) - fdefs of current parent type (to whom the subtypes belong).
 * @param opts (object) - Options object w.  fldinfo (for lookup of child types), layout: rowform/entform
*/
